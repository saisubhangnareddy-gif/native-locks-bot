...
async function scanChannel({ token, geminiKey, model, channel }) {
  ...
  // Keep only threads active within the window, most-recent first, and cap the
  // number analyzed per run so we stay within serverless time + Gemini quota.
  const MAX_THREADS = Number(process.env.MAX_THREADS || 15);
  ...
  // Analyze sequentially with pacing to respect Gemini free-tier RPM limits
  // (e.g. 15 rpm on flash-lite). GEMINI_DELAY_MS controls spacing between calls.
  const delayMs = Number(process.env.GEMINI_DELAY_MS || 4500);

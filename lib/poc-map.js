...
const PEOPLE = {
  ...
  // The nudger bot's OWN Slack user id — used to exclude its own nudge posts when
  // deciding "did anyone reply since our last nudge" (its posts must NOT count as
  // activity, or the 3-strike stop / consecutive-nudge logic never fires).
  BOT:          "U0BHEHNU0P7", // Native Locks Escalations (this bot)
  ...
  JYOTHI:       "U09MKC8333Q", // Jyothi Prakash Reddy — former Pro POC (no longer routed/cc'd; kept for direct handoffs)
  ...
  // POD QA — added to cc on a RE-NUDGE when a tagged agent hasn't responded.
  YOGESH:       "U066W93S3TM", // Yogesh Tomar — POD QA
...
// Readable display names by Slack ID — used for the "Blocked on" row where we
// show plain names (no @mentions). Only the @mentions in the action line ping.
const NAME_BY_ID = {
  [PEOPLE.SUBHANG]: "Subhang Reddy",
  [PEOPLE.SITA_RAM]: "Sita Ram",
  [PEOPLE.MANURANJAN]: "Manuranjan",
  [PEOPLE.HARSHA]: "Harshavardhan V",
  [PEOPLE.JYOTHI]: "Jyothi Prakash Reddy",
  [PEOPLE.ABHIRAM]: "Abhiram Dasika",
  [PEOPLE.GAGAN]: "Gagandeep",
  [PEOPLE.KUNAL]: "Kunal Chhonkar",
  [PEOPLE.BHASKAR]: "Bhaskar",
  [PEOPLE.YOGESH]: "Yogesh Tomar",
  [PEOPLE.ROHIT_BISHT]: "Rohit Singh Bisht",
  [PEOPLE.SHARVAN]: "Sharvan Negi",
  [PEOPLE.PADMANABHAN]: "Padmanabhan",
  [PEOPLE.JANMAYJAY]: "Janmayjay Sharma",
  [PEOPLE.VADLARAJU]: "Vadlaraju",
  [PEOPLE.MANOHAR]: "Manohar",
  [PEOPLE.SHAIKH]: "Shaikh Amirul",
  [PEOPLE.CHANDAN]: "Chandan Thakur",
  [PEOPLE.ANEK]: "Anek",
};
function nameById(id) { return NAME_BY_ID[id] || null; }
...
// Resolve ANY mapped person by free-text name (from the analyzer's "next_actor"
// field) to a Slack ID. Broader than smeIdForName — covers POD, RCA, logistics,
// engineering, leadership. Returns null for unmapped roles like "agent"/"PX".
const NAME_TO_ID = [
  ["subhang", PEOPLE.SUBHANG],
  ["sita", PEOPLE.SITA_RAM],
  ["manuranjan", PEOPLE.MANURANJAN],
  ["harsha", PEOPLE.HARSHA],
  ["jyothi", PEOPLE.JYOTHI],
  ["abhiram", PEOPLE.ABHIRAM],
  ["gagan", PEOPLE.GAGAN],
  ["kunal", PEOPLE.KUNAL],
  ["bhaskar", PEOPLE.BHASKAR],
  ["rohit", PEOPLE.ROHIT_BISHT],
  ["bisht", PEOPLE.ROHIT_BISHT],
  ["sharvan", PEOPLE.SHARVAN],
  ["padmanabhan", PEOPLE.PADMANABHAN],
  ["janmay", PEOPLE.JANMAYJAY],
  ["vadla", PEOPLE.VADLARAJU],
  ["manohar", PEOPLE.MANOHAR],
  ["shaikh", PEOPLE.SHAIKH],
...
// Role words the analyzer may return for next_actor when the person who owes the
// next step is NOT a mapped POC (they're the escalation agent / field PX). We
// can't @mention them by ID, so the composer renders a plain-text role label.
const ROLE_LABELS = {
  agent: "the escalation agent",
  requester: "the escalation agent",
  px: "the field PX",
  logistics: "logistics",
};
function roleLabelForName(name) {
  const n = (name || "").toLowerCase().trim();
  return ROLE_LABELS[n] || null;
}

// SKU-based primary RCA routing.
//   Harshavardhan is the sole primary RCA POC for ALL SKUs (Pro + Ultra +
//   any other). Jyothi is no longer the Pro POC — removed from routing/cc.
function rcaPocsForSku(sku) {
  return [PEOPLE.HARSHA];
}
...
const BLOCKERS = {
  ...
  entity_reset: {
    ...
    // Owner is whichever engineer was tagged for the reset — Gagan OR Abhiram.
    // The composer sets ctx.resetEngineer from the thread; default Gagan.
    pocs: (ctx) => [ctx && ctx.resetEngineer === "abhiram" ? PEOPLE.ABHIRAM : PEOPLE.GAGAN],
  ...
  reverse_pickup_pending: {
    ...
    // Proms → Jyothi (took over Proms reverse-pickup after Padmanabhan left);
    // otherwise warehouse (Sharvan).
    pocs: (ctx) => (ctx.pickupDest === "proms" ? [PEOPLE.JYOTHI] : [PEOPLE.SHARVAN]),
...
const CRITICAL_SIGNALS = {
  lockout: {
    ...
    // Tight lockout phrases only — the CUSTOMER cannot get into their home.
    // Deliberately NOT "not opening"/"unable to open" (almost every lock issue
    // says that) and NOT "standing outside" (matches two-way-talk "person
    // standing outside the door"). A lockout needs an explicit locked-out phrase
    // about the CX being unable to enter.
    keywords: ["lockout", "locked out", "cannot get inside", "can't get in", "cannot enter their home", "can't enter the house", "cx is lockout", "cx lockout", "customer is locked", "unable to enter the house"],
  },
  social_media_legal: {
    label: "Social-media / legal / consumer-court threat",
    pocs: [PEOPLE.MANURANJAN, PEOPLE.KUNAL],
    // Specific phrases only — bare "media"/"legal" substring-matched innocuous
    // words (e.g. "Delhivery", "illegal") and fired false criticals.
    keywords: ["social media", "consumer court", "consumer forum", "legal action", "legal notice", "will sue", "twitter", "instagram", "linkedin", "defamation", "court case", "go to media", "post on social", "media par"],
  },
};
...
module.exports = { PEOPLE, SMES, BLOCKERS, CRITICAL_SIGNALS, CRITICAL_CC, DEFAULT_CC, rcaPocsForSku, smeIdForName, personIdForName, roleLabelForName, nameById };

...
async function scanChannel({ token, apiKey, model, provider, channel }) {
  ...
  // under cron's ~30s timeout). 8 is a cap, not a guarantee — with mistral-large
  // (250K TPM, no rate-limit throttling) the limiter is wall-clock time, so a
  // typical run clears ~5-6 threads and the rest roll to the next run.
  const RUN_MAX = Number(process.env.RUN_MAX_THREADS || 8);
  ...
  // pace: mistral-large allows 1 req/sec; 1000ms keeps us safely under that while
  // roughly doubling throughput vs the old 2000ms idle gap.
  const paceMs = Number(process.env.GROQ_PACE_MS || 1000);
  const budgetMs = Number(process.env.RUN_BUDGET_MS || 24000); // ~6s headroom under a 30s timeout to post nudges

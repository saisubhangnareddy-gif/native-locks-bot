...
async function scanChannel({ token, groqKey, model, channel }) {
  ...
  // Analyze with bounded concurrency to fit many threads inside the serverless
  // time limit. Groq allows 30 RPM; the 429-aware retry in analyzeThread backs
  // off if we burst too hard. Wall-clock budget is a last-resort safety net.
  ...
  async function analyzeGuarded(item) {
    if (Date.now() - startedAt > budgetMs) { stoppedEarly = true; return; }
    analyzedCount++;
    await analyzeOne(item);
    // Mark done for this cycle so chained/next runs don't re-analyze it.
    try { await markAnalyzedThisCycle(channel, item.threadTs); } catch {}
  }
  await runPool(allThreads, concurrency, analyzeGuarded);

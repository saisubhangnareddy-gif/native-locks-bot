...
const { composeNudge, getNudgeState, setNudgeState, isRenudge, isAnalyzedThisCycle, analyzedSetThisCycle, markAnalyzedThisCycle, saveOpenEscalation, clearOpenEscalation, trackThread, untrackThread, getTrackedThreads } = require("./nudge");
...
// GROUND-TRUTH consecutive-nudge counter. Instead of trusting a stored counter
// (which resets if any prior run failed to persist, or was posted by an older
// code version), we count the bot's OWN nudge posts at the TAIL of the thread,
// walking backwards and STOPPING at the first human/intake message. This is
// immune to KV/state/deploy history: whatever is actually in the thread is the
// truth. If the tail has >= MAX consecutive bot nudges with no human message
// between them, the thread has been nudged enough — stop. Works even for nudges
// posted days ago. Trailing non-nudge bot rows (rare) are skipped, not counted.
function trailingNudgeCount(messages) {
  if (!Array.isArray(messages) || !messages.length) return 0;
  let count = 0;
  for (let i = messages.length - 1; i >= 0; i--) {
    const m = messages[i];
    const t = (m && m.text || "").trim();
    if (!t) continue;                 // skip empty/attachment-only rows
    if (isBotMessage(m)) { count++; continue; }  // our own nudge
    break;                            // first human OR intake message → stop
  }
  return count;
}
...
async function scanChannel({ token, apiKey, model, provider, channel }) {
  ...
  //
  // CRITICAL PERF: this was a sequential `await isAnalyzedThisCycle` per root —
  // O(N) KV round-trips. With a 300+ thread backlog that's ~30s of latency
  // BEFORE any LLM call, which blew the timeout and killed every cron run. Now
  // one batched, parallel MGET resolves the whole set in ~1 wave.
  const analyzedSet = await analyzedSetThisCycle(channel, activeRoots);
  const pendingAll = activeRoots.filter((root) => !analyzedSet.has(root));
  // RUN_MAX is the per-run CAP; the real governor is RUN_BUDGET_MS below (the
  // loop stops adding threads once the budget is spent, keeping the request
  // under cron's ~30s timeout). Default 2 is tuned for mistral-medium's LOW
  // 20K tokens/minute ceiling: at ~5-10K tokens/thread, ~2-3 threads/min is the
  // most medium can sustain without 429s. A high cap only makes sense on a model
  // with a much larger TPM (e.g. ministral-14b at 937K, or Groq gpt-oss-120b) —
  // raise RUN_MAX_THREADS via env if you switch to one of those.
  const RUN_MAX = Number(process.env.RUN_MAX_THREADS || 2);
  const pending = pendingAll.slice(0, RUN_MAX);

  // Runs are frequent (every ~1-5 min), so only a light in-run pace is needed.
  // CRITICAL: the whole request (scan + POST nudges) must finish INSIDE the
  // scheduler's HTTP timeout (~30s on cron-job.org, and Vercel may cut sooner) —
  // nudges are posted by the handler AFTER scanChannel returns, so if we run past
  // the timeout the function is killed BEFORE anything posts (→ no nudges at all).
  // Keep the scan budget well under 30s so there's headroom to post.
  const paceMs = Number(process.env.GROQ_PACE_MS || 2000);
  const budgetMs = Number(process.env.RUN_BUDGET_MS || 20000); // leave ~10s of the 30s timeout for posting
  ...
  let parkedCount = 0;
  let staleCount = 0;
  let stopped3Strike = 0;
  let errorCount = 0;
  let analyzedCount = 0;
  let skippedCooldown = 0;
  let lastError = null;
  let lastParseRaw = null;
  let lastParseThread = null;
  ...
  async function analyzeOne({ threadTs, messages }, nameOf) {
    // GROUND-TRUTH 3-STRIKE STOP (FIRST, before anything else). Count the bot's
    // own nudges at the tail of the thread. If we've already posted >= MAX with
    // NO human message since, stop — no analyze, no LLM, no post. This reads the
    // ACTUAL thread, so it catches over-nudged threads regardless of stored state
    // or when the nudges were posted (even days ago). The moment a human posts,
    // the tail count resets to 0 and nudging resumes.
    const MAX_UNANSWERED = Number(process.env.MAX_UNANSWERED_NUDGES || 3);
    const tailNudges = trailingNudgeCount(messages);
    if (tailNudges >= MAX_UNANSWERED) { skippedCooldown++; stopped3Strike++; return; }

    // Cooldown guard — if we nudged this thread recently, skip it entirely this
    // run (no analyze, no post). Cheap and saves an LLM call.
    ...
    // "error": the LLM response failed to parse — DO NOT nudge (a garbled/"could
    // not parse" nudge is noise). Skip this run, keep the thread tracked so the
    // next scheduled run retries with a fresh (hopefully clean) response.
    if (analysis.status === "error") {
      errorCount++;
      lastError = "parse_error";
      // Surface the raw model output + which thread, so we can diagnose the parse
      // failure from the sweep response without hunting for a thread_ts.
      lastParseRaw = analysis.raw || null;
      lastParseThread = threadTs;
      return;
    }
    ...
    // (The 3-strike stop already ran at the top via trailingNudgeCount — the
    // ground-truth thread count, which supersedes any stored streak. We keep
    // consecutiveNoMsg only to drive the action-only re-nudge rendering below.)

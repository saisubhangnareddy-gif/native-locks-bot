...
async function scanChannel({ token, groqKey, model, channel }) {
  ...
  async function analyzeOne({ threadTs, messages }) {
    ...
    if (analysis.status === "closed") {
      closedCount++;
      // Resolved -> stop tracking and clear its open record.
    ...
    // Persist open escalation (for the digest) AND track it persistently so we
    // keep nudging until it's resolved, even if it later goes silent past 3 days.
    try {

...
const { composeNudge, getNudgeState, setNudgeState, isRenudge, isAnalyzedThisCycle, markAnalyzedThisCycle } = require("./nudge");
...
async function scanChannel({ token, groqKey, model, channel }) {
  ...
  // ALL threads active within the window (parent may be months old, but if the
  // latest activity is within LOOKBACK_DAYS it counts).
  ...
  // CHUNKING: skip roots already analyzed in this cycle (tracked in KV with a
  // TTL), then take a bounded batch that fits in one serverless invocation.
  // Consecutive/self-chained runs cover the rest until the cycle is complete.
  const pending = [];
  ...
  // Fetch thread reply chains (only for this batch) with limited concurrency.
  const allThreads = [];
  ...
  await runPool(batch, 5, fetchRoot);
  ...
  // Groq 8B free tier is limited to ~6,000 tokens/MINUTE. Each thread analysis
  // uses ~1.5-2K tokens, so we can only do ~3 calls/min. Run STRICTLY serial
  // with pacing (default 20s) so we never burst past TPM. The 429-aware retry in
  // analyzeThread absorbs any residual rate-limit. A small batch keeps the run
  // within the serverless time budget; chained runs cover the rest.
  const paceMs = Number(process.env.GROQ_PACE_MS || 20000);
  const budgetMs = Number(process.env.RUN_BUDGET_MS || 285000); // < 300s limit
  ...
  for (let idx = 0; idx < allThreads.length; idx++) {
    ...
    try { await markAnalyzedThisCycle(channel, allThreads[idx].threadTs); } catch {}
    if (idx < allThreads.length - 1) await new Promise((r) => setTimeout(r, paceMs));
  }

  // Threads still pending in this cycle = those we didn't pull into this batch,
  // plus any in-batch ones we couldn't finish before the budget stopped us.
  const unfinishedInBatch = stoppedEarly ? Math.max(0, allThreads.length - analyzedCount) : 0;
  const remaining = remainingAfterBatch + unfinishedInBatch;

  results._stats = {
    totalActive,
    batchFetched: allThreads.length,
    analyzed: analyzedCount,
    closed: closedCount,
    open: results.filter((r) => r.nudgeText).length,
    errors: errorCount,
    stoppedEarly,
    remaining,                 // threads left to analyze in this cycle

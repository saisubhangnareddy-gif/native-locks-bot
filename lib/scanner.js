...
async function scanChannel({ token, apiKey, model, provider, channel }) {
  ...
  // under cron's ~30s timeout). Default 8 assumes a HIGH-TPM model like
  // ministral-14b-2512 (937K TPM) that can absorb ~8 x ~8K-token requests
  // without throttling. DO NOT run 8 on mistral-medium (20K TPM) — that blows
  // the per-minute limit and 429s. In practice the time budget clears ~5-6
  // threads/run; the rest roll to the next run.

...
async function scanChannel({ token, groqKey, model, channel }) {
  ...
  // Process as many pending threads as fit within the serverless time budget,
  // ALL IN THIS ONE INVOCATION (no fragile self-chaining). Groq 8B free tier is
  // ~6,000 tokens/min, so we pace serial calls. Whatever we can't finish stays
  // pending for the next run (cron or manual) — nothing is lost.
  const paceMs = Number(process.env.GROQ_PACE_MS || 22000);

...
// hub" mid-flow.
const RESOLVED_RE = /\b(issue resolved|resolved.*(px visited|visit done)|(px visited|visit done).*resolved|cx confirmed.*(resolved|working|fine)|working (fine|now|properly) now|case closed|now resolved|problem (is )?resolved)\b/i;

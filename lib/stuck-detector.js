...
const STANDALONE_CLOSE_RE = /^[\s>*_~`]*((this|it|its|it's|that|case|the case|the issue|issue|lock|problem|everything|all)\s+(is|has been|have been|been|got|now|now is)?\s*)?(resolved|closed|fixed|sorted|working (fine|now|properly)|done and (resolved|closed)|closing this|closure)([\s.,!✅✔️🔒👍🙏]|and\s+(closed|working)|,?\s*(closed|resolved|working)|\s+now)*[\s>*_~`]*$/i;

...
async function analyzeThread({ messages, nameOf, apiKey, model, provider }) {
  ...
  const body = {
    model: modelId,
    temperature: 0,
    // 800 is plenty for our compact JSON (~350 tokens of content). 1200 let the
    // model ramble ~2x longer per thread, which pushed each 3-thread run past the
    // cron/Vercel timeout → the function was killed before posting → no nudges.
    // Env-configurable so it can be tuned without a redeploy.
    max_tokens: Number(process.env.LLM_MAX_TOKENS || 800),

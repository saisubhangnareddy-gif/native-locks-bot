...
const STANDALONE_CLOSE_RE = /^[\s>*_~`]*((this|it|its|it's|that|case|the case|the issue|issue|lock|problem|everything|all)\s+(is|has been|have been|been|got|now|now is)?\s*)?(resolved?|closed|fixed|sorted|working (fine|well|now|properly)|done and (resolved?|closed)|closing this|closure)([\s.,!✅✔️🔒👍🙏]|and\s+(closed|working)|,?\s*(closed|resolved?|working)|\s+now|\s+well)*[\s>*_~`]*$/i;
...
async function analyzeThread({ messages, nameOf, apiKey, model, provider }) {
  ...
  const body = {
    model: modelId,
    temperature: 0,
    max_tokens: 1200,   // was 700 — long threads truncated the JSON mid-string, causing parse failures
  ...
  try {
    ...
    // Parse failed (usually a truncated/garbled response). DO NOT nudge on this —
    // a "Could not parse; review manually" nudge is noise. Return an error status
    // the scanner skips (no post, keep tracked so the next run retries cleanly).
    return { status: "error", parse_error: true, raw: rawText ? rawText.slice(0, 200) : null };
  }

...
// fulfillment step is handed to the warehouse regardless of a row number.
const SHEET_FILLED_RE = /\b((replacement|return|spare)\s+(sheet|form)\s+(filled|has been filled|been filled|is filled)|(filled|filling)\s+(the\s+)?(replacement|return|spare)\s+(sheet|form)|(sheet|form)\s+filled\s+(for|row))\b/i;
...
const REPLACEMENT_APPROVED_RE = /\b(replacement approved|approved.*replacement|replacement.*approved|please share (the )?tat|share (the )?tat with (the )?cx|share (the )?tat for the replacement)\b/i;
...
function isReplacementApprovedTail(messages) {
  ...
  for (let i = messages.length - 1; i >= 0; i--) {
    const t = (messages[i] && messages[i].text || "").trim();
    if (t) { lastText = t; lastIdx = i; break; }
  }
  if (!lastText) return false;
  // Park only when the FINAL substantive message is the approval / share-TAT
  // step itself. If a later message raised a new issue, do NOT park.
  if (!REPLACEMENT_APPROVED_RE.test(lastText)) return false;

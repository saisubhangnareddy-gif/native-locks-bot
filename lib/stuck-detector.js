...
async function analyzeThread({ messages, nameOf, apiKey, model, provider }) {
  ...
  const modelId = model || (isMistral ? "mistral-medium-2508" : "llama-3.3-70b-versatile");

  // gpt-oss (Groq) is a REASONING model: it emits hidden "thinking" tokens that
  // count against the completion budget and uses `max_completion_tokens` (not
  // `max_tokens`). If we leave the 1200 cap, reasoning can eat the whole budget
  // and truncate/empty the JSON — the exact failure we just fixed. So for gpt-oss
  // we (a) switch to max_completion_tokens, (b) give a larger budget, and
  // (c) set reasoning_effort:"low" — this is a routine classification, not a hard
  // reasoning task, so minimal thinking keeps output fast and JSON intact.
  const isGptOss = /gpt-oss/i.test(modelId);
  const maxTokens = Number(process.env.LLM_MAX_TOKENS || (isGptOss ? 4000 : 1200));
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    ...
    json = await res.json();
    // EMPTY-200 RETRY: Mistral occasionally returns HTTP 200 with an empty
    // message.content (no error field) when it's under load. That's transient —
    // giving up immediately turns a recoverable blip into a "parse_error" and a
    // skipped nudge. If we got a 200 with no usable content and attempts remain,
    // wait briefly and try the whole call again; only the LAST attempt's empty
    // response falls through to the error path below.
    const _c = json && json.choices && json.choices[0] && json.choices[0].message && json.choices[0].message.content;
    if ((!_c || !_c.trim()) && !(json && json.error) && attempt < 2) {
      await new Promise((r) => setTimeout(r, 800 * (attempt + 1)));
      json = null; // force another attempt
      continue;
    }
    break;
  }
  ...
  // Mistral does NOT always use OpenAI's {error:{message}} shape. On rate limits
  // / validation / capacity errors it returns a 200-or-4xx body with NO `choices`
  // and its OWN envelope: {object:"error", message, type} or {detail:...}. We
  // were mis-reading those as an "empty response". Detect any choice-less body
  // and throw the REAL message (so lastError shows e.g. "rate limit exceeded"
  // instead of a generic empty). The empty-retry above already gave it up to 3
  // attempts, so reaching here means it persisted — surface why and move on
  // (thread stays tracked; next scheduled run retries).
  if (!json.choices || !json.choices.length) {
    const emsg =
      (json.message && (typeof json.message === "string" ? json.message : JSON.stringify(json.message))) ||
      json.detail ||
      (json.object === "error" ? JSON.stringify(json) : null) ||
      `no choices in response: ${JSON.stringify(json).slice(0, 300)}`;
    throw new Error(`${provLabel} error: ${emsg}`);
  }
  ...
  try {
    ...
    if (salvaged && salvaged.status) {
      ...
      // Nothing usable — skip this run (no nudge), retry next run. Emit a compact
      // FINGERPRINT of the model envelope so an empty/garbled response is
      // self-diagnosing on the next run instead of a guess:
      //   finish_reason: stop|length|content_filter|none|no-choices
      //   content:       "string"|null|no-message  (null vs "" distinguishes
      //                  a refusal from an overloaded empty)
      //   usage:         completion tokens (0 => model emitted nothing at all)
      const choice = json && json.choices && json.choices[0];
      const fr = choice ? (choice.finish_reason == null ? "none" : choice.finish_reason) : "no-choices";
      const contentType = choice && choice.message
        ? (choice.message.content === null ? "null" : (choice.message.content === "" ? "empty-string" : typeof choice.message.content))
        : "no-message";
      const ct = (json && json.usage && (json.usage.completion_tokens ?? json.usage.total_tokens)) ?? "?";
      return { status: "error", parse_error: true, raw: rawText ? rawText.slice(0, 500) : `(empty response; finish_reason=${fr}; content=${contentType}; completion_tokens=${ct})` };
    }
  }

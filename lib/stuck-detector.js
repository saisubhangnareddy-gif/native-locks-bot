...
async function analyzeThread({ messages, nameOf, geminiKey, model }) {
  ...
  for (let attempt = 0; attempt < 4; attempt++) {
    ...
    if (res.status === 429 || res.status === 503) {
      // Respect the API's retry hint if present, else back off progressively.
      let waitMs = 4000 * (attempt + 1);
      try {
        const errJson = await res.clone().json();
        const msg = errJson.error && errJson.error.message ? errJson.error.message : "";
        const m = msg.match(/retry in ([\d.]+)s/i);
        if (m) waitMs = Math.ceil(parseFloat(m[1]) * 1000) + 500;
      } catch {}
      if (attempt === 3) { json = await res.json(); break; } // give up -> surface error
  ...
  if (!json) throw new Error("Gemini error: no response");
  ...
  try {
    ...
    // Strip code fences and extract the first {...} JSON object if wrapped.

...
- "near_closure": true ONLY if the fix has ACTUALLY BEEN DONE and just needs a customer confirmation — the replacement/spare was INSTALLED, the revisit was COMPLETED ("visit done"), the fix was applied and works. A visit that is merely ALIGNED / SCHEDULED / "px will visit today" is NOT near_closure — the work hasn't happened yet, so it's still an open revisit (revisit_eta_pending). A pending 2nd replacement or an install visit not yet done is NOT near_closure. It is still OPEN (not yet confirmed), and the natural next question is "is this now resolved?". Otherwise false. Do NOT set this while real work is still pending (parts not delivered, revisit scheduled-but-not-done, RCA open). CRITICAL EXCEPTION: if a NEW / different problem is raised in the LAST message(s) AFTER the original fix (e.g. mortise was replaced and "visit done", then cx reports "lock keeps going offline" — a fresh issue), near_closure MUST be false. A new tail issue means the thread is ACTIVE on that new problem — set the blocker/next_actor to the NEW issue, not "confirm the old fix".

...
async function analyzeThread({ messages, nameOf, anthropicKey, model }) {
  ...
  const body = {
    ...
    messages: [{ role: "user", content: `THREAD TRANSCRIPT:\n${transcript}\n\nReturn the JSON now.` }],
  };
  ...
  let parsed;
  try {
    const raw = json.content[0].text.trim().replace(/^```json\s*|\s*```$/g, "");

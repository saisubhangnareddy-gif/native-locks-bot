...
function renderTranscript(messages, nameOf) {
  ...
  const KEEP_RECENT = Number(process.env.KEEP_RECENT_MSGS || 14);
  let msgs = messages;
  let omittedNote = "";
...
- "steps_taken": <=220 char — what has been done or recommended SO FAR, synthesized across all replies in order (e.g. "PX revisit done 30 Jul; mortise re-seated; still failing, so replacement sheet filled row 1531"). If nothing substantive was done yet, say so plainly.

...
// A message posted by the nudger bot itself (its own nudge). These must be
// IGNORED when finding the "last substantive message" and when scanning the tail
// — otherwise the bot's own "please … to move this forward" nudge text trips the
// attention/needs-reply checks and blocks parking / closure. Detect three ways
// (any one): bot_id (present on app/bot-token posts), the bot's user id, OR the
// unmistakable shape of OUR nudge text (a robust fallback if bot_id is absent).
...
function isBotMsg(m) {
  ...
  if (m.user && m.user === PEOPLE.BOT) return true;
  if (m.text && OUR_NUDGE_RE.test(m.text)) return true;
  return false;
}
...
// (esp. Subhang) closes threads with a bare "resolved" / "closed". A
// long sentence that merely mentions "resolved" mid-flow must NOT match, so this
// only fires when the whole (stripped) message is basically the close word(s):
// "resolved", "resolved.", "resolved ✅", "closed", "issue closed".
const STANDALONE_CLOSE_RE = /^[\s>*_~`]*(issue\s+)?(resolved|closed|closing this|closure)([\s.,!✅✔️🔒👍🙏]|and\s+closed|,?\s*closed|,?\s*resolved|\s+now)*[\s>*_~`]*$/i;
...
function isResolvedTail(messages) {
  if (!Array.isArray(messages) || !messages.length) return false;
  // Look at the last few non-empty messages for a clear resolution statement.
  // We scan back a bit (not just the last 2) because a trailing bot/system row
  // or a short "thanks" can sit after the real "resolved" and push it out of a
  // 2-message window. A bare "resolved"/"closed" from anyone counts.
  const tail = messages.slice(-5).map((m) => (m && m.text) || "");
  return tail.some((tx) => RESOLVED_RE.test(tx) || isStandaloneClose(tx));
}
...
const IN_MOTION_RE = /\b(tracking id|tracking no|tracking number|awb|dispatched (on|today|tomorrow|yesterday)?|has been dispatched|been dispatched|out for delivery|shipped|courier|visit aligned|visit created|revisit created|revisit aligned|installation aligned|px aligned|visit (scheduled|booked) (on|for))\b/i;
...
async function analyzeThread({ messages, nameOf, apiKey, model, provider }) {
  // Strip the bot's OWN nudge posts up front. In auto mode the bot posts nudges
  // INTO the thread; those must not count as thread activity — otherwise the
  // "last substantive message" is the bot's own "please … to move this forward"
  // nudge, which trips the attention/needs-reply checks and blocks parking and
  // closure. Every deterministic detector below runs on human messages only.
  const humanMsgs = Array.isArray(messages) ? messages.filter((m) => !isBotMsg(m)) : messages;

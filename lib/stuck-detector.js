...
CRITICAL — rca_not_closed is ONLY for SOFTWARE/FIRMWARE root-cause investigations that Engineering owns:
firmware bugs, battery drain/leakage (power management), PCB/mother-PCB electronic failure, camera/live-video,
motion-sensor logic, Wi-Fi/notification-signal debugging, "under RCA by backend/engineering".
DO NOT use rca_not_closed for these — pick a different blocker instead:
- HARDWARE / MECHANICAL build issues (accessory plate, anti-pry plate, weld beads, front-panel fitment, gasket,
  strike plate, mortise, springs, physical alignment, intruder/anti-pry alarm from fitment) => revisit_not_aligned
  or install_quality (these are SME/PX field fixes, NOT firmware). Tag SME/PX, never the firmware engineer.
- PX damaged the door / property damage / poor installation / unprofessional behaviour => install_quality
- Customer wants a supervisor/manager call, refund demand, service dissatisfaction => awaiting_diagnosis (or social/legal if threatened)
- Delivery / tracking / pickup logistics => delivery_delay / replacement_delivery_pending / reverse_pickup_pending
Only use rca_not_closed when Engineering/firmware is genuinely investigating a SOFTWARE/electronic root cause.
When unsure, do NOT default to rca_not_closed.
...
async function analyzeThread({ messages, nameOf, apiKey, model, provider }) {
  ...
  const modelId = model || (isMistral ? "mistral-medium-latest" : "llama-3.1-8b-instant");

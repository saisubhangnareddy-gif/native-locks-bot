...
async function analyzeThread({ messages, nameOf, apiKey, model, provider }) {
  ...
  try {
    ...
    if (salvaged && salvaged.status) {
      ...
      // Nothing usable — skip this run (no nudge), retry next run. Capture the
      // model's finish_reason so an empty/garbled response tells us WHY (e.g.
      // "length" = truncated, "content_filter" = refused, "" = overloaded).
      const fr = (json.choices && json.choices[0] && json.choices[0].finish_reason) || "";
      return { status: "error", parse_error: true, raw: rawText ? rawText.slice(0, 500) : `(empty response${fr ? `; finish_reason=${fr}` : ""})` };
    }
  }

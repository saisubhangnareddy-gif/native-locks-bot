...
function scanCritical(threadText) {
  ...
  // BEFORE replacement. Replacement is the resolution path, so once it's
  // underway (sheet filled / approved / delivered / reverse pickup / post-
  // replacement RCA) the customer is no longer stuck outside; drop the lockout
  // critical. (Social-media/legal criticals are NOT stage-gated.)
  if (hits.includes("lockout")) {
    const pastReplacement = /(sheet fill|sheet filled|replacement approved|replacement.*deliver|deliver.*replacement|reverse pick|pick.*after replacement|tracking id|proms for rca|replacement done|replacement lock installed)/i.test(t);
    if (pastReplacement) {
...
// hub" mid-flow.
const RESOLVED_RE = /\b(issue resolved|resolved.*(px visited|visit done)|(px visited|visit done).*resolved|cx confirmed.*(resolved|working|fine)|working (fine|now|properly) now|case closed|now resolved|problem (is )?resolved|take (a )?clos(ure|er)|please close this case|cx arranged.*(carpenter|technician).*(changed|replaced|fixed|installed))\b/i;
...
- "near_closure": true if the issue appears ESSENTIALLY DONE and only a final confirmation is missing — e.g. the replacement/spare was installed, the revisit was completed, the fix was applied, and the only remaining step is confirming with the customer that it's working. It is still OPEN (not yet confirmed), but the natural next question is simply "is this now resolved?". Otherwise false. Do NOT set this while real work is still pending (parts not delivered, revisit not done, RCA open). CRITICAL EXCEPTION: if a NEW / different problem is raised in the LAST message(s) AFTER the original fix (e.g. mortise was replaced and "visit done", then cx reports "lock keeps going offline" — a fresh issue), near_closure MUST be false. A new tail issue means the thread is ACTIVE on that new problem — set the blocker/next_actor to the NEW issue, not "confirm the old fix". Do not ask to confirm resolution when the latest message reports a fresh fault.

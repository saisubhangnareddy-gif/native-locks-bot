...
function extractTailHandoff(messages) {
  ...
  // Match by bot_id, the bot's user id, or the shape of our nudge text.
  const OUR_NUDGE_RE = /(\*Action needed\*|Escalation summary|:lock: \*CRITICAL|:rotating_light: \*CRITICAL|to move this forward)/i;
  messages = messages.filter((m) => !(m && (m.bot_id || m.user === PEOPLE.BOT || (m.text && OUR_NUDGE_RE.test(m.text)))));

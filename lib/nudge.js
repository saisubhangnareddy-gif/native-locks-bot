...
function composeNudge({ analysis, isRenudge, participants }) {
  ...
  const nextActorId = idForName(analysis.next_actor);   // person B (agent or POC), or null
  const askedById = idForName(analysis.asked_by);       // person A, or null
  const roleLabel = roleLabelForName(analysis.next_actor); // "the field PX" etc. when unmapped
  const isHandoff = !!analysis.handoff;
  const nextAction = (analysis.next_action_needed && String(analysis.next_action_needed).trim()) || "";

  let primaryTags;      // who we @tag on the action line
  let addressLabel = ""; // plain-text label when person B isn't tag-able (e.g. field PX)
  let actionAsk = ask;

  if (isHandoff && (nextActorId || roleLabel)) {
    // Tag person B (the actor). cc = owner/SKU POC + person A + Subhang.
    const ownerCc = [...pocs];
    ...
    if (nextActorId) {
      ...
      primaryTags = tagAll(pocs);
      addressLabel = roleLabel;
      actionAsk = `could you get ${addressLabel} to ${needAsAction(need)}?`;
    }
  } else if (nextActorId && !pocs.includes(nextActorId)) {
    // No explicit handoff, but a specific in-thread actor clearly owes the next
  ...
  if (isCritical) {
    ...
    // Critical POCs join the chase unless a specific live actor already owns it.
    if (!isHandoff && !(nextActorId && !resolvePocs(def, ctx).includes(nextActorId))) {
      primaryTags = tagAll(pocs);
    } else {

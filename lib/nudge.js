...
function composeNudge({ analysis, isRenudge, participants, messages, blockedIds }) {
  ...
  // ASK source: when the tail extractor fired, its action comes from the ACTUAL
  // latest request line — that is the CURRENT step (ground truth). The model's
  // next_action_needed can be stale (anchored on a superseded earlier stage), so
  // for tail-fired cases we prefer the tail action. Only when the tail did NOT
  // ASK source: prefer the MODEL's next_action_needed — it is an intelligent,
  // reframed, closure-oriented instruction (the prompt forces it to reflect the
  // CURRENT tail step, not a stale one, and to improve on the raw line rather
  // than parrot it). The tail extractor's raw line is only a fallback when the
  // model gave nothing. WHO to tag still comes from the tail extractor.

...
function composeNudge({ analysis, isRenudge, participants }) {
  ...
  // Resolve a display name to a Slack ID: prefer an ACTUAL thread participant
  // (so we only ever tag people who posted here), then fall back to the mapped
  // POC roster. Returns null for role words / unknown names.
  const idForName = (name) => {
    const n = (name || "").toLowerCase().trim();
    if (!n) return null;
    if (parts[n]) return parts[n];
    // loose participant match (first-name / substring) before the POC map
    for (const key of Object.keys(parts)) {
      if (key.includes(n) || n.includes(key)) return parts[key];
    }
    return personIdForName(name);
  };
  ...
  // next_actor may be an array of names, a single name string, or null. Resolve
  // each to a Slack ID (thread participant first, then POC map). Keep any that
  // resolve; note whether any addressee was an unmappable role word (PX etc).
  const rawActors = Array.isArray(analysis.next_actor)
    ? analysis.next_actor
    : (analysis.next_actor ? [analysis.next_actor] : []);
  const actorIdsAll = [...new Set(rawActors.map(idForName).filter(Boolean))];
  const roleLabel = rawActors.map(roleLabelForName).find(Boolean) || null; // first PX/logistics role, if any
  const askedById = idForName(analysis.asked_by);       // person A, or null
  ...
  // NEVER be tagged to act. Drop the asker's id from the actor set. If that
  // empties the set, the direction is unreliable — fall back to the owner.
  const actorIds = askedById ? actorIdsAll.filter((id) => id !== askedById) : actorIdsAll;

  let primaryIds = [];   // who we @tag on the action line
  let addressLabel = ""; // plain-text label when person B isn't tag-able (e.g. field PX)
  let actionAsk = ask;

  if (isHandoff && (actorIds.length || roleLabel)) {
    // Tag person(s) B (the actor(s)). cc = owner/SKU POC + person A + Subhang.
    const ownerCc = [...pocs];
    if (askedById) ownerCc.push(askedById);
    cc = [...new Set([...cc, ...ownerCc])];

    const need = nextAction || "the pending input (invoice / sheet / confirmation)";
    if (actorIds.length) {
      primaryIds = [...actorIds];
      actionAsk = `please ${needAsAction(need)} to move this forward.`;
    } else {
      // Unmapped actor (field PX / logistics) — can't @tag, address in text and
      // keep the owner tagged so someone accountable drives it.
      primaryIds = [...pocs];
      addressLabel = roleLabel;
      actionAsk = `could you get ${addressLabel} to ${needAsAction(need)}?`;
    }
    ...
    // Default: chase the blocker owner / SKU POC.
    primaryIds = [...pocs];
  }
...
// phrase for the ask. If it already starts with a verb, leave it.
function needAsAction(need) {
  const n = need.trim();
  if (/^(share|upload|confirm|add|send|provide|create|align|check|update|complete|initiate|fill)\b/i.test(n)) {

...
// CONDITIONAL / SEPARATED IMPERATIVE: a mention, then some words (often a
// condition like "if knob is working smoothly then"), then a directive verb on
// the SAME line — e.g. "@Manju if knob is working smoothly then fill spare sheet
// for mortise". The addressed person owes that step even though the verb isn't
// tight against the mention. Kept to strong action verbs to avoid false hits.
const TAIL_COND_IMPERATIVE = /<@[A-Z0-9]+>[^\n]{0,80}?\b(create|align|fill|initiate|punch|schedule|dispatch|deliver|replace|reset|arrange|raise|connect|confirm|pick|install|send|coordinate|complete)\b/i;
...
function extractTailHandoff(messages) {
  ...
  // Ignore the bot's OWN nudge posts — otherwise the extractor parses the bot's
  // own "@X please … to move this forward" line as a fresh handoff and loops.
  // Match the nudge TEXT shape (not bot_id) so escalation INTAKE headers posted
  // by the same app are NOT stripped (they carry no handoff anyway).
  ...
  messages = messages.filter((m) => !(m && m.text && !INTAKE_HEADER_RE.test(m.text) && OUR_NUDGE_RE.test(m.text)));
  ...
  for (let i = messages.length - 1; i >= start; i--) {
    ...
    const isRequest = TAIL_REQUEST.test(text) || TAIL_IMPERATIVE.test(text) || TAIL_COND_IMPERATIVE.test(text) || completion;
    ...
    // Conditional / separated imperative: grab from the directive verb onward,
    // even if a condition sits between the mention and the verb ("@Manju if knob
    // is working smoothly then fill spare sheet for mortise" → "fill spare sheet
    // for mortise").
    if (!pm) pm = text.match(/\b((?:create|align|fill|initiate|punch|schedule|dispatch|deliver|replace|reset|arrange|raise|connect|confirm|pick|install|send|coordinate|complete)\b[^\n.]{0,140})/i);
    if (pm && pm[1]) {
      action = pm[1]
...
function composeNudge({ analysis, isRenudge, actionOnly, participants, messages, blockedIds }) {
  ...
  const ctx = {
    ...
    // Entity-reset owner = whichever engineer was tagged for the reset. If
    // Abhiram (alfadelta10010) is in the thread, the reset was routed to him;
    // otherwise default to Gagan. Drives entity_reset.pocs in the map.
    resetEngineer: Object.values(parts).includes(PEOPLE.ABHIRAM) ? "abhiram" : "gagan",
  };
  ...
  // If the entity reset is ALREADY DONE (someone posted "entity has been reset"
  // / "reset done"), the case has moved to the operational config step — do NOT
  // force-tag Gagan (he may never have been in the thread; Abhiram often does the
  // reset). Defer to the tail handoff (e.g. "@Vikash @Rushali reset & configure").
  const resetDone = Array.isArray(messages) && messages.some((m) =>
    /\b(entity (has been|is) reset|entity reset done|reset (has been )?done|reset is done|already reset)\b/i.test((m && m.text) || "")
  );
  const engineeringForce =
    (analysis.blocker === "deep_firmware_rca" || (analysis.blocker === "entity_reset" && !resetDone))
    && !tailActorMapped;
  if (engineeringForce) {
    primaryIds = [...pocs];   // pocs = [Abhiram] or [Gagan] for these blockers
    actionAsk = def.ask;
  }
  // Reset-done cleanup: if the model still returned entity_reset but the reset is
  // complete, the blocker owner must NOT stay Gagan. Move Blocked-on to the SKU
  // POC (the config retry is now an operational/RCA step) so the row + cc don't
  // show a stale Engineering owner.
  if (analysis.blocker === "entity_reset" && resetDone) {
    pocs = rcaPocsForSku(ctx.sku);
    blockerLabel = "Entity reset done — lock reset + configuration retry pending";
  }
  ...
  //   • SKU POC — Harshavardhan (all SKUs: Pro + Ultra) — they track issues.
  ...
  // TAGGED AGENT → add Yogesh Tomar (POD QA) to cc, on EVERY nudge (not just
  // re-nudges). If the tagged actor is an ESCALATION AGENT — someone NOT in the
  // mapped POC/logistics/SME set — loop in POD QA for oversight. Only when
  // Yogesh's real id is configured (skip the placeholder).
  if (PEOPLE.YOGESH && !/^U_/.test(PEOPLE.YOGESH)) {
    const taggedAgent = primaryIds.some((id) => !mappedIds.has(id));
    if (taggedAgent) cc = [...new Set([...cc, PEOPLE.YOGESH])];
  }
  ...
  const lines = [];
  // FIRST nudge: full summary (header + Issue/Steps/Status/Blocked-on). Action-only
  // ONLY when this is a CONSECUTIVE nudge with NO message of any kind in the thread
  // since our last nudge (actionOnly) — the summary above in the thread already
  // shows the SKU + issue, so repeating even the header line just clutters it. On a
  // consecutive nudge we emit ONLY the "Action needed" block + cc. Falls back to
  // isRenudge if the scanner didn't pass actionOnly.
  const stripSummary = (typeof actionOnly === "boolean") ? actionOnly : isRenudge;
  if (!stripSummary) {
    // Title: critical banner if applicable, else neutral header. SKU appended.
    // Distinct emoji per critical type: :lock: lockout, :rotating_light: legal.
    if (isCritical) {
...
// BATCHED cycle-check. The scanner must decide, for a LIST of thread roots,
// which are already analyzed this cycle. Doing one `get` per root is O(N)
// SEQUENTIAL KV round-trips — with a 300+ thread backlog that's ~30s of pure
// network latency BEFORE any LLM work, which blew the request timeout and
// killed every cron run (no nudges posted). Instead we MGET in chunks and run
// the chunks in PARALLEL, collapsing ~300 round-trips into a handful (~1 wave).
// Returns a Set of threadTs strings that are already analyzed this cycle.
// If KV is unconfigured/errors, returns an EMPTY set (treat all as pending) so
// the run still makes forward progress rather than dying.
async function analyzedSetThisCycle(channel, threadTsList) {
  const set = new Set();
  if (!Array.isArray(threadTsList) || !threadTsList.length) return set;
  const CHUNK = 40; // keep each MGET URL well under length limits
  const chunks = [];
  for (let i = 0; i < threadTsList.length; i += CHUNK) {
    chunks.push(threadTsList.slice(i, i + CHUNK));
  }
  try {
    const waves = await Promise.all(
      chunks.map(async (chunk) => {
        const keys = chunk.map((ts) => `cycle:${channel}:${ts}`);
        const vals = await kv("mget", ...keys);
        return { chunk, vals };
      })
    );
    for (const { chunk, vals } of waves) {
      if (!Array.isArray(vals)) continue; // null => unconfigured; treat as pending
      chunk.forEach((ts, idx) => { if (vals[idx]) set.add(ts); });
    }
  } catch {
    return new Set(); // on any error, treat everything as pending
  }
  return set;
}
...
module.exports = { composeNudge, getNudgeState, setNudgeState, isRenudge, tagAll, isAnalyzedThisCycle, analyzedSetThisCycle, markAnalyzedThisCycle, saveOpenEscalation, clearOpenEscalation, getAllOpenEscalations, trackThread, untrackThread, getTrackedThreads };

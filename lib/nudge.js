...
function composeNudge({ analysis, isRenudge, participants, messages, blockedIds }) {
  ...
  // (Abhiram / Gagan) with the clean standard ask — never the escalation-header
  // POCs the model may pick. BUT if a genuine later handoff fired (the tail
  // extractor found someone asked to do a NEXT operational step AFTER the RCA —
  // e.g. "@Padmanabhan align battery", "@Rushali connect with cx"), the RCA
  // phase is over and that handoff wins. Only force Engineering when there is no
  // such later handoff.
  if ((analysis.blocker === "deep_firmware_rca" || analysis.blocker === "entity_reset") && !tail) {

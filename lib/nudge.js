...
function composeNudge({ analysis, isRenudge, participants, messages }) {
  ...
  // When the tail extractor fired, the ACTION must come from that SAME line
  // (not the model's possibly-stale next_action_needed) so tag and ask agree.
  const nextAction = tail && tail.action
    ? tail.action
    : (analysis.next_action_needed && String(analysis.next_action_needed).trim()) || "";

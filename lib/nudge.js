...
function composeNudge({ analysis, isRenudge, actionOnly, participants, messages, blockedIds }) {
  ...
  // FIRST nudge: full summary. Action-only ONLY when this is a CONSECUTIVE nudge
  // with NO message of any kind in the thread since our last nudge (actionOnly) —
  // repeating Issue/Steps/Status/Blocked-on into a silent thread just clutters it.
  // If anyone posted in between, actionOnly is false and we show the full summary
  // again (the context may have changed). Falls back to isRenudge if the scanner
  // didn't pass actionOnly.

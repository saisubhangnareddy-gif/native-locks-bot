...
const { BLOCKERS, CRITICAL_SIGNALS, PEOPLE, DEFAULT_CC, CRITICAL_CC, personIdForName, roleLabelForName, rcaPocsForSku } = require("./poc-map");
...
// --- DETERMINISTIC HANDOFF EXTRACTOR (code-first; model is the fallback) ------
// Scans the thread's raw messages for the LATEST "@X please <verb> ..." request
// and returns { actorIds, askedById, actionText } straight from Slack's real
// <@UID> mentions — no name-resolution guesswork. This is ground truth for who
// was asked to act. Returns null if no clear request line exists (then the
// composer falls back to the model's next_actor / blocker owner).
const REQUEST_VERB = /\b(please|pls|kindly|help|share|confirm|fill|create|align|connect|check|update|initiate|punch|schedule|dispatch|deliver|replace|reset|arrange|raise|approve|inform|look into|revisit|pickup|pick)\b/i;
// Lines that look like a request but are really just relays / non-actionable.
const NON_ACTION = /\b(any update|thank you|thanks|noted|okay|ok\b|done\b|informed|closed|fyi)\b/i;

function extractHandoff(messages) {
  if (!Array.isArray(messages) || !messages.length) return null;
  // Walk from the newest message backward; the first genuine "@X please Y"
  // request wins (that's the current pending handoff).
  for (let i = messages.length - 1; i >= 0; i--) {
    const m = messages[i];
    const text = (m.text || "").trim();
    if (!text) continue;
    const mentions = [...text.matchAll(/<@([A-Z0-9]+)>/g)].map((x) => x[1]);
    if (!mentions.length) continue;                 // no addressee, not a handoff
    if (!REQUEST_VERB.test(text)) continue;          // not phrased as a request
    if (NON_ACTION.test(text) && !/\bplease|pls|kindly\b/i.test(text)) continue;
    // The addressees are the people asked (person B). The author is person A.
    // Drop self-mentions and the author from the actor set.
    const actorIds = [...new Set(mentions.filter((id) => id !== m.user))];
    if (!actorIds.length) continue;
    return { actorIds, askedById: m.user || null, actionText: text.slice(0, 200) };
  }
  return null;
}

// Compose the thread reply text. Format: SUMMARY first, then ACTIONABLE.
// `handoffOverride` (from the deterministic extractor) takes precedence over the
// model's next_actor/asked_by when present.
function composeNudge({ analysis, isRenudge, participants, handoffOverride }) {
  ...
  const modelActorIds = [...new Set(rawActors.map(participantId).filter(Boolean))];
  const modelAskedById = idForNameLoose(analysis.asked_by);

  // HYBRID: prefer the deterministic extractor's result (real <@UID> mentions
  // from the actual request line). Fall back to the model only when the code
  // found no clear "@X please Y" handoff.
  const useOverride = handoffOverride && handoffOverride.actorIds && handoffOverride.actorIds.length;
  const actorIdsAll = useOverride ? [...handoffOverride.actorIds] : modelActorIds;
  const askedById = useOverride ? handoffOverride.askedById : modelAskedById;
  const isHandoff = useOverride ? true : !!analysis.handoff;
  ...
  // --- cc composition (per instruction): cc = the SKU POC(s) + Subhang only.
  // NOT the full mapped POC set, not person A, not the blocker owner. Tracking
  // is owned by the SKU RCA POC (Jyothi+Harsha for Pro, Harsha for Ultra); on
  // critical we also cc Sita Ram. Anyone who is the PRIMARY tag is removed from
  // cc later so nobody appears twice.
  const skuPocs = rcaPocsForSku(ctx.sku);
  cc = [...new Set([...DEFAULT_CC, ...skuPocs])]; // Subhang + SKU POC(s)
  ...
  if (isCritical) {
    ...
    cc = [...new Set([...cc, ...CRITICAL_CC])]; // + Sita Ram (critical ONLY)
  }
...
// Build a grammatical ask from next_action_needed. To avoid double-verbs like
...
module.exports = { composeNudge, extractHandoff, getNudgeState, setNudgeState, isRenudge, tagAll, isAnalyzedThisCycle, markAnalyzedThisCycle, saveOpenEscalation, clearOpenEscalation, getAllOpenEscalations, trackThread, untrackThread, getTrackedThreads };

...
function composeNudge({ analysis, isRenudge, participants, messages, blockedIds }) {
  ...
  // ASK source — coherence first:
  //   • Tail fired  → use the TAIL action. It comes from the SAME request line as
  //     the tagged person, so tag and ask always describe the same current step.
  //     (Using the model's action here risks a stale step, e.g. tagging Neha for
  //     "share invoice" but asking her to "check the debugger".)
  //   • Tail did NOT fire → use the MODEL's action (an intelligent reframe of the
  //     current step). Falls back across both if one is empty.

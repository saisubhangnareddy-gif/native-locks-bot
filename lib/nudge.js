...
function needAsAction(need) {
  const n = need.trim();
  if (/^(share|upload|confirm|add|send|provide|create|align|check|update|complete|initiate|fill|punch|schedule|dispatch|replace|reset|arrange|raise|close)\b/i.test(n)) {

...
// A message counts as a request if it has a politeness cue OR a bare imperative
// directly after a mention ("@Rushali create revisit"). Kept tight to avoid
// false positives (the old blunt extractor's failure mode).
...
// Bare imperative: a mention immediately followed by an action verb.
const TAIL_IMPERATIVE = /<@[A-Z0-9]+>[\s,:-]*(create|align|share|fill|check|update|initiate|punch|schedule|dispatch|deliver|replace|reset|arrange|raise|connect|confirm|pick|install|send|add|coordinate|complete)\b/i;
...
function extractTailHandoff(messages) {
  ...
  for (let i = messages.length - 1; i >= start; i--) {
    ...
    if (!text) continue;
    const isRequest = TAIL_REQUEST.test(text) || TAIL_IMPERATIVE.test(text);
    ...
    let action = "";
    let pm = text.match(/\b(?:please|pls|kindly|need you to|needs to|we need to|can you|could you|requesting you|request you)\b[\s,:-]*([^\n.]{3,140})/i);
    if (!pm) pm = text.match(/<@[A-Z0-9]+>[\s,:-]*((?:create|align|share|fill|check|update|initiate|punch|schedule|dispatch|deliver|replace|reset|arrange|raise|connect|confirm|pick|install|send|add|coordinate|complete)\b[^\n.]{0,140})/i);
    if (pm && pm[1]) {
      action = pm[1]
...
function composeNudge({ analysis, isRenudge, participants, messages, blockedIds }) {
  ...
  // ASK source — the MODEL writes the ask (a short, crisp, reframed instruction;
  // capped and told to reflect the CURRENT step, never to copy raw thread text).
  // The tail extractor's raw line is ONLY a fallback when the model gave nothing
  // — never preferred, because raw lines drag in rambling customer VOC / mid-
  // sentence fragments and make the nudge look dumb. WHO to tag still comes from
  // the tail extractor.
  const modelAction = (analysis.next_action_needed && String(analysis.next_action_needed).trim()) || "";
  const nextAction = modelAction || (tail && cleanRawAction(tail.action)) || "";
  ...
  // For Engineering RCA blockers (deep_firmware_rca / entity_reset), the pending
  // step is often a fuzzy back-and-forth ("what needs validating?"). Use the
  // blocker's CLEAN standard ask instead of a rambling model reframe, AND force
  // the primary tag to the blocker POC (Gagan for entity_reset, Abhiram for
  // deep_firmware_rca) — never the escalation-header POCs the model may have
  // picked. Engineering owns these; they get tagged via the blocker, not as a
  // handoff.
  if (analysis.blocker === "deep_firmware_rca" || analysis.blocker === "entity_reset") {

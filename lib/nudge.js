...
const { BLOCKERS, CRITICAL_SIGNALS, PEOPLE, SMES, DEFAULT_CC, CRITICAL_CC, personIdForName, roleLabelForName, rcaPocsForSku } = require("./poc-map");
...
// A message counts as a request if it has one of these cues. Kept tight to
// avoid false positives (the old blunt extractor's failure mode).
const TAIL_REQUEST = /\b(please|pls|kindly|need you to|needs to|we need to|can you|could you|requesting you|request you|kindly help)\b/i;
// Requests that are NOT a real actionable handoff (withdrawal / no-op).
const TAIL_NONACTION = /\b(ignore|no need|nvm|never ?mind|disregard)\b/i;
const TAIL_LOOKBACK = 3;
function extractTailHandoff(messages) {
  ...
  for (let i = messages.length - 1; i >= start; i--) {
    ...
    if (!text || !TAIL_REQUEST.test(text)) continue;
    if (TAIL_NONACTION.test(text)) continue;   // "please ignore" etc. — not a handoff
    const mentions = [...text.matchAll(/<@([A-Z0-9]+)>/g)].map((x) => x[1]);
    if (!mentions.length) continue;
    const EXCLUDE = new Set([PEOPLE.SUBHANG, PEOPLE.ABHIRAM, PEOPLE.GAGAN, m.user]);
    const actorIds = [...new Set(mentions.filter((id) => !EXCLUDE.has(id)))];
    if (!actorIds.length) continue;
    // Pull the ACTION text from THIS SAME line so the tag and the ask always
    // describe the same pending step (the model's next_action_needed may be
    // anchored on a stale earlier stage). Take the clause after the request cue.
    let action = "";
    const pm = text.match(/\b(?:please|pls|kindly|need you to|needs to|we need to|can you|could you|requesting you|request you)\b[\s,:-]*([^\n.]{3,140})/i);
    if (pm && pm[1]) {
      action = pm[1]
...
function composeNudge({ analysis, isRenudge, participants, messages }) {
  ...
  const tail = extractTailHandoff(messages);
  ...
  // WHO comes from the extractor (reliable). The ASK comes from the MODEL — it
  // reframes the pending step into a crisp, closure-oriented question rather
  // than parroting the thread's last line. We only fall back to the extracted
  // line if the model gave nothing usable.
  const modelAction = (analysis.next_action_needed && String(analysis.next_action_needed).trim()) || "";
  const nextAction = modelAction || (tail && tail.action) || "";
  ...
  const excludeFromActor = new Set([PEOPLE.SUBHANG, PEOPLE.ABHIRAM, PEOPLE.GAGAN]);
  ...
  // --- cc composition: SKU POC(s) + Subhang, PLUS any mapped POC who is ACTUALLY
  // INVOLVED in this thread (posted in it). We do NOT cc the full static POC set
  // or uninvolved owners — e.g. Manuranjan is cc'd only when he actually acted in
  // the thread. Engineering (Abhiram/Gagan) and Sita Ram are handled by their own
  // rules below and excluded here. Anyone who is the PRIMARY tag is removed later.
  const skuPocs = rcaPocsForSku(ctx.sku);
  const participantIdSet = new Set(Object.values(parts));
  // SMEs are relevant ONLY to revisit-audit / PX-quality cases. On any other
  // blocker, an SME who happened to post early should NOT linger in cc.
  const smeAllowed = analysis.blocker === "revisit_audit_quality";
  const mappedInvolved = Object.values(PEOPLE).filter(
    (id) => participantIdSet.has(id)
      && id !== PEOPLE.ABHIRAM && id !== PEOPLE.GAGAN
      && id !== PEOPLE.SITA_RAM && id !== PEOPLE.SUBHANG
      && (smeAllowed || !SMES.includes(id))
  );
  cc = [...new Set([...DEFAULT_CC, ...skuPocs, ...mappedInvolved])];
  ...
  if (isCritical) lines.push(`:rotating_light: *CRITICAL — ${criticalLabel}*${skuTag}${followup}`);

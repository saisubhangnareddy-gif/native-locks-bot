...
function composeNudge({ analysis, isRenudge, participants }) {
  ...
  // RULE: whoever must ACT NEXT gets TAGGED. The model returns next_actor (the
  // person[s] asked to do the pending step) and asked_by (who asked them).
  // next_actor may be an array, a single name string, or null. It is resolved
  // against THREAD PARTICIPANTS ONLY — a name the model invents (not in the
  // thread) resolves to nothing and we fall back to the blocker owner, rather
  // than mis-tagging a POC by fuzzy name match.
  ...
  const askedById = idForNameLoose(analysis.asked_by);  // person A, or null

...
function composeNudge({ analysis, isRenudge }) {
  ...
  if (analysis.critical && analysis.critical.length) {
    ...
    pocs = def.pocs.slice();
    blockerLabel = def.label;
    if (!ask) {
      ask = key === "lockout"
        ? "please treat as top priority and confirm the next step / PX ETA."
  ...
  const flag = isCritical ? ":rotating_light: " : "";
  const followup = isRenudge ? " · _following up again, still no closure_" : "";

  // SUMMARY block first, then ACTIONABLE block.
  const lines = [];
  lines.push(`${flag}*Escalation summary*${followup}`);
  if (analysis.one_line) lines.push(`• *What's happening:* ${analysis.one_line}`);
  if (analysis.last_activity) lines.push(`• *Latest update:* ${analysis.last_activity}`);
  lines.push(`• *Blocked on:* ${blockerLabel} — ${tagAll(pocs)}`);
  lines.push("");
  lines.push(`*Action needed* — ${tagAll(pocs)}: ${ask}`);
  lines.push(`_cc ${tagAll(cc)} · reply "resolved" here once closed so I stop nudging._`);
